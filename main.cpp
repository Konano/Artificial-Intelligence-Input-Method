...
{	
	...
	for(int o=0; o<len; o++)
	{
		...
		for(int i=0; i<num_2; i++) for(int j=0; j<num_1; j++) for(int k=0; k<num_0; k++)
			tot_3 += Times_3[Long(Pinyin_vec[text[o-2]][j], Pinyin_vec[text[o-1]][i], Pinyin_vec[text[o]][i])]+1;
				...
				for(int j=0; j<num_1; j++)
				{
					...
					for(int k=0; k<num_0; k++)
					{
						double tmp = f[o-1][k*num_1+j] + 
									 log(1.0*(Times_3[Long(Pinyin_vec[text[o-2]][k], Pinyin_vec[text[o-1]][j], Pinyin_vec[text[o]][i])]+1) / tot_3) +
									 log(1.0*(Times_2[Int(Pinyin_vec[text[o-1]][j], Pinyin_vec[text[o]][i])]+1) / tot_2) +
									 log(1.0*(Times_1[Pinyin_vec[text[o]][i]]+1) / tot_1);
